...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        /// 
        ///This method is a recursive method designed to visit each dependent all the way through every branch. The purpose
        ///behind this is to firstly locate all cells that need updating, but also to thouroughly check for CircularExceptions.
        ///
        /// Basically, what happens is we start with a given cell, and we check its direct dependents. If it finds itself in that list, it's a CircularException.
        /// If not, we recurse by going into an unvisited cell and begin looking at all of its dependents. If we find the original cell there,
        /// CircularException. Otherwise, keep recursing until every single cell that is a direct OR indirect dependent of the original cell has been visited.
        /// </summary>
